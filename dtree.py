...
def _d_build_tree_string(root, curr_index, index=False, delimiter='-'):
    ...
    l_box, l_box_width, l_root_start, l_root_end = \
        _d_build_tree_string(root.left, 2 * curr_index + 1, index, delimiter)
    r_box, r_box_width, r_root_start, r_root_end = \
        _d_build_tree_string(root.right, 2 * curr_index + 2, index, delimiter)
...
def search(root, test_array): 
    ...
    if test_array[root.dim] < root.value: 
        return search(root.left, test_array)
    else:
    # Key is smaller than root's key 
        return search(root.right, test_array) 


class DNode(Node):
    def __init__(self, value, dim=None, left=None, right=None):
        super().__init__(value, left, right)
        self.dim = dim
        self.weight = 0
        
    def __str__(self):
        lines = _d_build_tree_string(self, 0, False, '-')[0]
